...
The skill stops at the artifact, and there is no auto-dispatch mode. Dispatch is manual and, inside one Deliverable, serial: run [implement](https://aihero.dev/skills-implement) on the first Subtask, [clear](https://www.aihero.dev/ai-coding-dictionary/clearing), run it on the next. Everything lands on one branch and [land-the-work](https://aihero.dev/skills-land-the-work) opens one merge request at the end. Parallelism lives one level up: sibling Deliverables whose blockers are done can run at once, an agent each, since `implement` gives every Deliverable its own branch and its own worktree.
...
Upstream is [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec to slice against; keep both in one unbroken context window. Downstream is [implement](https://aihero.dev/skills-implement), which claims the Deliverable and builds one Subtask per fresh session, driving [tdd](https://aihero.dev/skills-tdd) for the tests and closing with [code-review](https://aihero.dev/skills-code-review). Then [land-the-work](https://aihero.dev/skills-land-the-work) puts the whole Deliverable on a branch and opens one merge request. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
