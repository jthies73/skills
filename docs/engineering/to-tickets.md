...
The skill stops at the artifact, and there is no auto-dispatch mode. Dispatch is manual and, inside one Deliverable, serial: run [implement](https://aihero.dev/skills-implement) on the first Subtask, [clear](https://www.aihero.dev/ai-coding-dictionary/clearing), run it on the next. Everything lands on one branch and [land-the-work](https://aihero.dev/skills-land-the-work) opens one merge request at the end. Parallelism lives one level up: sibling Deliverables whose blockers are done can run at once.
