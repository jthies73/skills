...
A role maps to **zero or more** labels, which is what lets the same five roles drive a kanban board: map both ready roles to a `TODO` column label plus their own name, and map `needs-triage` to a `Backlog` column label. `wontfix` maps to a real `wontfix` label plus closed, and a sixth, non-triage terminal role, `done`, maps to a real `Done` label plus closed; a scheduled reconciliation sweep keeps that pair honest rather than a human ritual. The roles and the skill are unchanged; only the right-hand column of the mapping is.
...
| `wontfix` | Closed, `wontfix` label applied, with the reason recorded. |
...
`wontfix` splits three ways, and the difference matters because only one of them writes to the knowledge base. Every path applies the `wontfix` label itself, named at the step that closes the issue rather than left to a general rule stated earlier in the skill:

| Why you're closing it | What happens |
| --- | --- |
| Already implemented | `wontfix` applied, then a comment pointing at where it already lives. Nothing is written to `.out-of-scope/`, because it's a built feature, not a rejected one, and filing it there would poison the dedup checks. |
| Rejected bug | `wontfix` applied, then a polite explanation, then close. |
| Rejected enhancement | `wontfix` applied, then a file in `.out-of-scope/`, linked from the closing comment, then close. |
