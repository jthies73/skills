...
`to-spec` turns the conversation you have just had into a **[spec](https://www.aihero.dev/ai-coding-dictionary/spec)**, and writes it into a single issue on your tracker: the **Deliverable** this work will be delivered as. Where you brought it an existing issue, that is the one it fills.
...
It doesn't anymore, and this was the most-reported rough edge on the skill. It used to label the spec ready on publish, which meant an [AFK](https://www.aihero.dev/ai-coding-dictionary/afk) agent polling for `ready-for-agent` would try to build the whole spec in one run instead of picking up the slices.

`to-spec` now leaves the state role alone. The Deliverable becomes ready when [to-tickets](https://aihero.dev/skills-to-tickets) has published its **Subtasks** and moves it there as its final step, so readiness means both "specified" and "split", which is the pair that makes it safe to grab. The one exception is a build small enough to skip `to-tickets` entirely: there `to-spec` applies the ready role itself and tells you it did.
