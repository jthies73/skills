...
Edit the right-hand column to match whatever vocabulary you actually use. A role can map to several labels, or to none at all: see [triage-labels-kanban.md](./triage-labels-kanban.md) for a board-shaped repo where `ready-for-agent` costs two labels and `needs-triage` costs none.
