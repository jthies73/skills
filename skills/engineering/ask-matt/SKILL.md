...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** takes the Deliverable's branch first, in a git worktree of its own next to the repo, and pushes it so the branch is on the host while the work is in flight, then builds each Subtask by driving **`/tdd`** internally (one red-green slice at a time), closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, and stops there. The worktree is what lets sibling Deliverables run at the same time, an agent each, with no shared HEAD and no shared index. **`/land-the-work`** is the step after it, run in that same worktree **once per Subtask**, before the next one starts: each reviewed slice goes into its own Conventional Commit on the branch already holding it. It asks before anything leaves your machine, every time. Say yes on the first Subtask and it pushes and opens the merge/pull request as a **draft**, titled from the Deliverable, with a description carrying the feature description and the reference that closes the ticket on merge; say yes on the last and it marks that draft ready for review, which is also when the card moves to Review. Say no and it stops at the commit. It is model-invoked, so the agent will usually reach for it once the work is finished, and it is worth reaching for by hand on any finished change that never came through a spec. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
