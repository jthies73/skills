...
**On a Soron repo, run `/setup-soron-skills` instead**, and only ever one of the two: they write the same three files. It is the same setup with every org-wide answer already given (GitLab, the board, group labels on `DEV`, single-context docs), so it asks only what genuinely varies per repo, verifies the nine group labels exist, and finishes by pointing you at [WORKFLOW.md](../setup-soron-skills/WORKFLOW.md). That guide is the one to read before your first card: the two levels, the six lists and who moves a card into each, and the rules that get broken most. Everything above this section is unchanged by it, because a board is still just a right-hand column in one mapping file.
