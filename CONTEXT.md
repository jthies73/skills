...
**Triage role**:
A canonical state-machine role an **Issue** occupies during triage (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`). Each role maps to **zero or more** real label strings in the **Issue tracker** via this repo's `.agents/triage-labels.md` (other repos use `docs/agents/triage-labels.md`). A role mapping to none is carried by the absence of a label, which is how the `enhancement` category is expressed.

**Terminal label**:
One of the two labels, `Done` or `wontfix`, that a closed **Issue** carries; an open Issue carries neither, and the two are mutually exclusive. A scheduled reconciliation sweep enforces the invariant, since a human closing an Issue by hand or a merge closing it via a keyword both leave the label to be derived rather than chosen. See [ADR-0004](.agents/adr/0004-terminal-labels-on-close.md), which reverses the earlier "Done is the closed state, not a label" convention: `Done` is now a **Triage role** label like any other, not an inference from the host's Closed list.
